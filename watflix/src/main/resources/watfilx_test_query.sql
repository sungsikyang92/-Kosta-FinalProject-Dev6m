...
SELECT P.POINT_HISTORY_NO,P.ID,P.REVIEW_NO,(select REVIEW_TITLE from REVIEW where P.REVIEW_no=REVIEW_no) AS REVIEW_TITLE,
      P.COMMENTS_NO,(select COMMENTS from COMMENTS where P.COMMENTS_no=COMMENTS_no) as COMMENTS,
      (select C.CONTENTS_NO from REVIEW R,CONTENTS C where P.REVIEW_no=R.REVIEW_NO and C.CONTENTS_NO=R.CONTENTS_NO) AS CONTENT_NO,
      (select C.CONTENTS_TITLE from REVIEW R,CONTENTS C where P.REVIEW_no=R.REVIEW_NO and C.CONTENTS_NO=R.CONTENTS_NO) AS CONTENT_TITLE,
      (select C.CONTENTS_NO,C.CONTENTS_TITLE from COMMENTS M,CONTENTS C where P.COMMENTS_no=M.COMMENTS_NO and C.CONTENTS_NO=M.CONTENTS_NO),
...
select QNA_NO,ID,QNA_TITLE,QNA_CONTENTS,QNA_POSTED_TIME
	from (select ROW_NUMBER() OVER(ORDER BY QNA_NO DESC) AS RNUM, ID, QNA_NO,QNA_TITLE,QNA_CONTENTS,QNA_POSTED_TIME FROM QNA WHERE id = 'java1')
